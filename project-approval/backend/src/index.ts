...
function isApproved(c: any, s: any) { const id = String(s?.id || s?.uuid || s?.statusID || s?.statusUUID || s || ''); const name = String(s?.name || s?.title || s?.statusName || ''); return (c.approved_status_uuid && id === c.approved_status_uuid) || (!c.approved_status_uuid && name.trim() === (c.approved_status_name || '已通过')) }
...
export async function onIssueStatusChanged(req: PluginRequest): Promise<PluginResponse> { const e = eventOf(req), teamUUID = String(e.ctx.teamID || e.ctx.teamUUID || e.data.teamID || team(req)); const issueID = String(e.data.issueID || e.data.issueUUID || e.data.id || ''); const status = e.data.newStatus || e.data.status || e.data.toStatus || e.data.targetStatus; if (!teamUUID || !issueID) return ok({ ignored: true }); const c = await config(teamUUID); if (!c.approval_project_uuid || !c.issue_type_uuid || !isApproved(c, status)) return ok({ ignored: true }); try { const issue = await issueDetail(teamUUID, issueID); const issueType = String(issue?.issue_type_uuid || issue?.issueTypeUUID || issue?.type_uuid || issue?.issueType?.uuid || ''); if (issueType && issueType !== c.issue_type_uuid) return ok({ ignored: true }); if (issue?.project_uuid && issue.project_uuid !== c.approval_project_uuid) return ok({ ignored: true }); return ok(await run(teamUUID, issueID, e.id)) } catch (err: any) { return ok({ status: 'failed', issue_uuid: issueID, error: err?.message || '创建项目失败' }) } }
