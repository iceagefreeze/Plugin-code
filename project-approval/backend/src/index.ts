...
async function createProject(teamUUID: string, input: any, templateUUID: string): Promise<string> {
  const r: any = await OPFetch({ url: `/project/api/project/team/${teamUUID}/projects`, method: 'POST', headers: { 'Content-Type': 'application/json' }, data: { name: input.name, description: input.description || '', owner: input.owner, start_time: input.start_time || 0, end_time: input.end_time || 0, template_uuid: templateUUID } })
...
async function run(teamUUID: string, issueUUID: string, eventID: string, retry = false): Promise<any> {
  ...
  try {
    ...
    const name = String(issue?.name || issue?.title || value('立项名称') || '').trim(); const owner = String(value('项目负责人') || issue?.assignee || '').trim()
    if (!name || !owner) throw Object.assign(new Error('立项单缺少项目名称或项目负责人'), { code: 'MISSING_FIELD' })
    const projectUUID = await createProject(teamUUID, { name, owner, description: String(value('立项说明') || value('描述') || ''), start_time: value('计划开始日期'), end_time: value('计划结束日期') }, c.template_uuid)
    const done = { issue_uuid: issueUUID, team_uuid: teamUUID, event_id: eventID || existing?.event_id || '', status: 'created', project_uuid: projectUUID, project_name: name, error_code: '', error_message: '', retry_count: Number(existing?.retry_count || 0) + (retry ? 1 : 0), updated_at: Date.now() }
...
export async function onIssueStatusChanged(req: PluginRequest): Promise<PluginResponse> { const e = eventOf(req), teamUUID = String(e.ctx.teamID || ''); if (!teamUUID || !e.data.issueID) return ok({ ignored: true }); const c = await config(teamUUID); if (!c.approval_project_uuid || !c.issue_type_uuid || !isApproved(c, e.data.newStatus)) return ok({ ignored: true }); const issue = await issueDetail(teamUUID, e.data.issueID); if (issue?.project_uuid && issue.project_uuid !== c.approval_project_uuid) return ok({ ignored: true }); try { return ok(await run(teamUUID, String(e.data.issueID), e.id)) } catch { return ok({ status: 'failed', issue_uuid: e.data.issueID }) } }
