...
export async function onIssueStatusChanged(req: PluginRequest): Promise<PluginResponse> { const e = eventOf(req), teamUUID = String(e.ctx.teamID || e.ctx.teamUUID || e.data.teamID || team(req)); const issueID = String(e.data.issueID || e.data.issueUUID || e.data.id || ''); const status = e.data.newStatus || e.data.status || e.data.toStatus || e.data.targetStatus; if (!teamUUID || !issueID || !isApproved({ approved_status_name: '已通过' }, status)) return ok({ ignored: true }); try { return ok(await run(teamUUID, issueID, e.id)) } catch (err: any) { return ok({ status: 'failed', issue_uuid: issueID, error: err?.message || '创建项目失败' }) } }
