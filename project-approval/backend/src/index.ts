...
function isApproved(c: any, s: any) { const id = String(s?.id || s?.uuid || s?.statusID || s?.statusUUID || s || ''); const name = String(s?.name || s?.title || s?.statusName || ''); const configured = String(c.approved_status_uuid || ''); return (configured ? id === configured : id === '3uLZYVM1' || name.trim() === (c.approved_status_name || '已通过')) }
...
export async function onIssueStatusChanged(req: PluginRequest): Promise<PluginResponse> { const e = eventOf(req), teamUUID = String(e.ctx.teamID || e.ctx.teamUUID || e.data.teamID || team(req)); const issueID = String(e.data.issueID || e.data.issueUUID || e.data.id || ''); const status = e.data.newStatus || e.data.status || e.data.toStatus || e.data.targetStatus; if (!teamUUID || !issueID || !isApproved({ approved_status_name: '已通过' }, status)) return ok({ ignored: true }); try { const result = await run(teamUUID, issueID, e.id); Logger.info(`[立项审批] 项目创建成功 issue=${issueID} project=${result.project_uuid}`); return ok(result) } catch (err: any) { const message = `${err?.code || 'CREATE_FAILED'}: ${err?.message || '创建项目失败'}${err?.detail ? `; ${err.detail}` : ''}`; Logger.error(`[立项审批] 项目创建失败 issue=${issueID} ${message}`); throw new Error(message) } }
export async function getConfig(req: PluginRequest): Promise<PluginResponse> { return ok({ approved_status_name: '已通过', approved_status_uuid: '3uLZYVM1', template_uuid: 'waterfall', mapping_json: '{}', ...(await config(team(req))) }) }
export async function saveConfig(req: PluginRequest): Promise<PluginResponse> { const teamUUID = team(req); if (!teamUUID) return fail('MISSING_TEAM', '无法识别当前团队，请从团队插件配置页打开'); const b = body(req); const approvalProject = normalizeId(b.approval_project_uuid); const issueType = normalizeId(b.issue_type_uuid); const template = normalizeId(b.template_uuid) || 'waterfall'; if (!approvalProject || !issueType || (!b.approved_status_uuid && !b.approved_status_name)) return fail('INVALID_CONFIG', '审批项目、工作项类型和通过状态均为必填'); let mapping = '{}'; try { if (typeof b.mapping_json === 'string') { JSON.parse(b.mapping_json); mapping = b.mapping_json.slice(0, 4096) } else if (b.mapping_json && typeof b.mapping_json === 'object') mapping = JSON.stringify(b.mapping_json).slice(0, 4096) } catch { return fail('INVALID_CONFIG', '属性映射格式无效') } const value = { team_uuid: teamUUID, approval_project_uuid: approvalProject, issue_type_uuid: issueType, approved_status_uuid: normalizeId(b.approved_status_uuid) || '3uLZYVM1', approved_status_name: String(b.approved_status_name || '已通过').trim(), template_uuid: template, mapping_json: mapping }; runtimeConfig[teamUUID] = value; try { await configs.set(key(teamUUID), value) } catch {} return ok(value) }
